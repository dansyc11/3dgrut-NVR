...
def quad_corners(center, hx, hy):
    return np.array([[center[0] + sx * hx, center[1] + sy * hy, center[2]] for sx in (-1, 1) for sy in (-1, 1)])
...
def run_scene(tag, cloud, center, normal, rig, ref):
    ...
    print(
        f"PASS  {tag}: verify_path (in-view, quadrants, bands, "
    ...
    assert np.array_equal(eyes[:lead_n], np.repeat(eyes[:1], lead_n, 0)), "positions move during the stationary lead"
    assert np.array_equal(R_wc[:lead_n], np.repeat(R_wc[:1], lead_n, 0)), "orientation moves during the stationary lead"
    assert not np.allclose(eyes[lead_n + int(5 * cfg.fps)], eyes[0]), "path never leaves the initial pose"
    ...
    wv = np.array([vt._rotvec(R2[i], R2[i + 1]) / dtf for i in range(len(R2) - 1)])
    ...
    assert (
        jerk[win].max() <= max(rest.max(), 1e-6) * 1.2 + 1e-6
    ), f"jerk spike at the boundary: {jerk[win].max():.3f} vs {rest.max():.3f}"
    assert (
        ajerk[win].max() <= max(arest.max(), 1e-6) * 1.2 + 1e-6
    ), f"angular jerk spike at the boundary: {ajerk[win].max():.3f}"
    print(
        f"PASS  {tag}: C2 - jerk max {jerk.max():.3f} m/s^3 "
