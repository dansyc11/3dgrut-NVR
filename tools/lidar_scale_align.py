...
Usage (--datasets, or VILOTA_DATASETS, is the folder that holds meetingroom/
and main_campus/; the result is written next to the inputs):
...
# Dataset paths are relative to the datasets folder (--datasets or VILOTA_DATASETS).
PRESETS = {
    "meetingroom": dict(
        splat="meetingroom/splat_points.ply",
        lidar="meetingroom/lidar_sub.npy",
    ...
    "campus": dict(
        ...
        lidar="main_campus/lidar_sub.npy",
        images_bin="main_campus/colmap/model/images.bin",
        out="main_campus/lidar_alignment.json",
        ...
        sfm_points="main_campus/colmap/model/points3D.bin",
        register_with="sfm",
...
def main():
    ...
    for key in ("splat", "lidar", "images_bin", "out", "sfm_points"):
        cfg[key] = os.path.join(args.datasets, cfg[key])
