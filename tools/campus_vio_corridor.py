...
def centreline(seats, gup):
    ...
    cents = np.array([ph[idx].mean(0) for idx in cells.values() if len(idx) >= 3])
    # end A = the seat furthest along +e1 (the (16.9, -3.6) end)
    start = cents[np.argmin(np.linalg.norm(cents - ph[np.argmax(uv[:, 0])], axis=1))]
    chain = [start]
    ...
    for _ in range(3):
        hline = np.convolve(np.pad(hline, 4, mode="edge"), np.ones(9) / 9, mode="valid")
...
def stadium(line, gup):
    ...
    def cap(centre, frm, outward, k=12):
        ...
        for th in np.linspace(0, math.pi, k + 2)[1:-1]:
            pts.append(centre + r * (math.cos(th) * ah + math.sin(th) * side))
        return np.array(pts)

    capB = cap(line[-1], out_leg[-1], t[-1])
    capA = cap(line[0], back_leg[-1], -t[0])
    pieces = [("legA", out_leg), ("capB", capB), ("legB", back_leg), ("capA", capA)]
    pts, spans, s0 = [], [], 0.0
    for kind, p in pieces:
        seg = np.linalg.norm(np.diff(p, axis=0), axis=1)
        pts.append(p)
        spans.append((kind, s0, s0 + seg.sum()))
        s0 += seg.sum() + np.linalg.norm((pieces + pieces[:1])[len(spans)][1][0] - p[-1])
...
def knot_poses(pts, s_arr, L, knots, gup, ev, kind):
    ...
    for s in knots:
        ...
        t = circuit_point(pts, s_arr, L, s + eps) - circuit_point(pts, s_arr, L, s - eps)
        t = horiz(t, gup)
        t /= np.linalg.norm(t)
        yaw = 0.0
        if kind == "stress":
            for st in ev["turns"]:
                ...
                if abs(d) < HEAD_TURN_HALF:
                    yaw = math.radians(HEAD_TURN_DEG) * (1.0 - abs(d) / HEAD_TURN_HALF)
            p = p + gup * (STRESS_BOB_AMP * math.sin(2 * math.pi * s / STRESS_BOB_WAVE))
        if kind == "extreme":
            ...
            if wlo < s < whi:
                ...
                off = 0.5 * EXT_WEAVE_AMP * (1.0 - math.cos(2 * math.pi * (s - wlo) / EXT_WEAVE_WAVE))
            ...
            p = p + gup * (lo_rel + (hi_rel - lo_rel) * 0.5 * (1.0 - math.cos(2 * math.pi * s / L)))
        if yaw:
...
def cmd_build(args):
    ...
    print(
        f"centreline one-way {L1:.1f} m, stadium circuit {L:.1f} m " f"({len(line)} samples, lane offset {LANE_OFF} m)"
    )
    ...
    board = []
    centre_h = horiz(seats, gup).mean(0) + (np.median(seats @ gup) + OVERHEAD_Y_M) * gup
    ...
    np.savetxt(
        ...
        board,
        delimiter=",",
        header="stand-in aim board, SCENE UNITS, %.1f m overhead; " "pass as --cloud WITH --alignment" % OVERHEAD_Y_M,
    )
    print(f"wrote {bpath}")

    with open(args.alignment) as f:
        raw = json.load(f)
    cam_above = raw["ground"]["splat"]["cameras_above_floor_units"] * align.scale
    ...
    for kind, h_real, stress, fps, name, prof in (
        ...
        (
            ...
            {"kind": "extreme", "v_peak": EXT_V_PEAK, "v_stop": EXT_V_STOP},
        ),
    ):
        ...
        if kind == "extreme":
            ...
            ev["h_band"] = (EXT_BAND_M[0] - cam_above, EXT_BAND_M[1] - cam_above)
        ...
        np.savetxt(
            ...
            fmt="%.6f",
            header=(
                f"{kind}: x,y,z [m, scene axes] + qx,qy,qz,qw "
...
def cmd_check(args):
    ...
    with open(args.alignment) as f:
        raw = json.load(f)
    cam_above_floor = raw["ground"]["splat"]["cameras_above_floor_units"] * align.scale
    ...
    print(f"  duration {dur:.1f} s = {lead:g} s lead + {dur - lead:.1f} s " f"motion, {len(p)} frames @ {fps:g} fps")
    print(f"  path length (motion) {length:.1f} m")
    print(
        f"  speed  m/s   peak {speed[mv].max():.2f}  p95 "
        f"{np.percentile(speed[mv], 95):.2f}  mean {speed[mv].mean():.2f}"
    )
    print(f"  accel  m/s^2 peak {a_mag[mv].max():.2f}  p95 " f"{np.percentile(a_mag[mv], 95):.2f}")
    print(f"  |f|    m/s^2 min {f_mag[mv].min():.2f}  max " f"{f_mag[mv].max():.2f}   (parked: {G:.2f})")
    print(f"  gyro  rad/s  peak {w[mv[:-1]].max():.2f}  p95 " f"{np.percentile(w[mv[:-1]], 95):.2f}")
    print(f"  nearest seat m   max {dmin.max():.2f}  median " f"{np.median(dmin):.2f}")
    print(
        f"  height above seat plane m  min {habove.min():.2f}  max "
...
def main():
    ...
    for name in ("build", "check"):
        ...
        s.add_argument(
            ...
            help="campus lidar_alignment.json " "(default $VILOTA_DATASETS/main_campus/lidar_alignment.json)",
        )
        if name == "build":
