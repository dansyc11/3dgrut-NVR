...
export VIO_WAYPOINTS=campus_vio/campus_baseline_waypoints.csv
...
#
# Rendering: with VIO_WAYPOINTS set (above), build_vio_trajectory - the
# GUI's "VIO trajectory" entry, or drive_render.py with VIO_TRAJ=1 -
# renders this waypoint circuit with the same phase clock as the
# dry-run, so the render's truth npz matches the dry-run's.
