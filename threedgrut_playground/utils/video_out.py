...
class VideoRecorder:
    ...
    def get_num_frames(self, list_len: int):
        ...
        if num_intervals <= 0:
            raise ValueError(
                "Cannot calculate number of frames for an empty trajectory or a trajectory with a single camera."
            )
    ...
    def render_linear_trajectory_to_png(self, interpolation_mode="catmull_rom", cam_name: str = "CamX"):
        ...
        distortions = (
            self.trajectory[0].distortion_coefficients
            if hasattr(self.trajectory[0], "distortion_coefficients")
            else None
        )
        intrinsics = (
            self.trajectory[0].get_camera_intrinsics() if hasattr(self.trajectory[0], "get_camera_intrinsics") else None
        )
        if not intrinsics:
            raise ValueError(
                "The first camera in the trajectory does not have intrinsic parameters. "
                "Ensure all cameras in the trajectory have valid intrinsic parameters."
            )
        interpolated_path = camera_path_generator(
            trajectory=self.trajectory, frames_between_cameras=self.frames_between_cameras, interpolation="catmull_rom"
        )
