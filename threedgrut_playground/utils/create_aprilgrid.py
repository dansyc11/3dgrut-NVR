...
def create_checkerboard_og(res: int, square: int) -> torch.Tensor:
    """
    Creates a checkerboard pattern with the specified resolution and square size.

    Args:
        res (int): The resolution of the checkerboard (number of squares along one side).
        square (int): The size of each square in pixels.

    Returns:
        torch.Tensor: A tensor representing the checkerboard pattern.
    """
    checkboard_res = res
    checkboard_square = square
    ...
    return checkboard_texture


def create_aprilgrid_texture(gap_ratio: float, square: int, device, rows=4, cols=7) -> torch.Tensor:
    """
    Creates a checkerboard pattern with the specified resolution and square size.
    ...
    Returns:
        torch.Tensor: A tensor representing the checkerboard pattern.
    """
    checkboard_gap = square * gap_ratio
    gaps_cols = checkboard_gap * (cols - 1)
    gaps_rows = checkboard_gap * (rows - 1)
    width = int(square * cols + gaps_cols)
    height = int(square * rows + gaps_rows)

    square = int(square)
    gap = int(checkboard_gap)

    tag_square = int(square * 0.5)

    checkboard_square = square
    checkboard_texture = torch.tensor([1.0, 1.0, 1.0, 1.0], device=device, dtype=torch.float32).repeat(height, width, 1)
    # Set the first 10*10 square to white\
    # start_square = 0
    checkboard_texture = create_grid(square, gap, 0.0, rows, cols, checkboard_texture)
    checkboard_texture = create_grid(
        tag_square, gap, 1.0, rows, cols, checkboard_texture, offset=int(square / 4), is_april=True, device=device
    )

    # for row in range(rows):
    #     for col in range (cols):
    #         start_row = row *int(square + checkboard_gap)
    #         #print(f"Start:{start_square}")
    #         end_row = start_row + square
    #         start_col = col * int(square + checkboard_gap)
    #         end_col = start_col + square
    #         #print(f"End:{end_square}")
    #         checkboard_texture[start_row:end_row, start_col:end_col, :3] = 0.0

    # start = int(square/4) + start_square
    # end = int(square - start) + start_square
    # checkboard_texture[start:end, start:end, :3] = 1.0
    # start_square += int(checkboard_gap + square)

    # padding
    checkboard_texture = add_padding(width, height, gap, checkboard_texture, device=device)
...
def add_padding(width, height, gap, checkboard_texture, device=torch.device("cuda:0")):
    padding_left = torch.tensor([1.0, 1.0, 1.0, 1.0], device=device, dtype=torch.float32).repeat(
        height + 2 * gap, gap, 1
    )
    padding_right = torch.tensor([1.0, 1.0, 1.0, 1.0], device=device, dtype=torch.float32).repeat(
        height + 2 * gap, gap, 1
    )
    padding_top = torch.tensor([1.0, 1.0, 1.0, 1.0], device=device, dtype=torch.float32).repeat(gap, width, 1)
    padding_bottom = torch.tensor([1.0, 1.0, 1.0, 1.0], device=device, dtype=torch.float32).repeat(gap, width, 1)
    vert_padded = torch.cat([padding_top, checkboard_texture, padding_bottom], dim=0)
    fully_padded = torch.cat([padding_left, vert_padded, padding_right], dim=1)
    return fully_padded
...
def create_grid(
    ...
    tag_order = [21, 22, 23, 24, 25, 26, 27, 14, 15, 16, 17, 18, 19, 20, 7, 8, 9, 10, 11, 12, 13, 0, 1, 2, 3, 4, 5, 6]
    index = 0
    for row in range(rows):
        for col in range(cols):
            ...
            end_col = start_col + square
            # print(f"End:{end_square}")
            # texture[start_row:end_row, start_col:end_col, :3] = color
            if is_april:
                curr_tag = tag_order[index]
                create_tag(start_row, end_row, start_col, end_col, square, texture, tagID=curr_tag, device=device)
                index += 1
            else:
                texture[start_row:end_row, start_col:end_col, :3] = color
...
def create_tag(
    ...
    tagFamililyData = AprilTagCodes("t16h5")
    try:
        tag_code = tagFamililyData.tagCodes[tagID]
    except:
        print(
    ...
    code_matrix = torch.tensor([1.0, 1.0, 1.0, 1.0], device=device, dtype=torch.float32).repeat(
        sqrt_bits * bit_square_size, sqrt_bits * bit_square_size, 1
    )
    matrix = torch.tensor([1.0, 1.0, 1.0, 1.0], device=device, dtype=torch.float32).repeat(sqrt_bits, sqrt_bits, 1)
...
def display_texture(texture: torch.tensor):
    img = texture.cpu().numpy()
    # If you just want to show the RGB part:
    plt.imshow(img[:, :, :3])  # ignore alpha
    plt.axis("off")
    plt.title("Checkboard Texture")
    plt.show()


def main():
    checker_tex = create_checkerboard(rows=4, cols=7, gap_ratio=0.3, square=80)
    # april_tag =
    display_texture(checker_tex)
