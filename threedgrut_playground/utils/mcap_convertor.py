...
Written for the vio-sim-offline replay path (vk_playback ->
...
def _check_rotation(R, what):
    ...
    if R.shape != (3, 3) or np.abs(R @ R.T - np.eye(3)).max() > 1e-3 or abs(np.linalg.det(R) - 1.0) > 1e-3:
...
class RigCalibration:
    ...
    def __init__(self, path):
        ...
        if not self.serial:
            ...
            print(
                f"[mcap_convertor] WARNING: {path} has no deviceName; " f"using frameId {self.serial!r}",
                file=sys.stderr,
            )
        ...
        ref_sockets = [
            s
            for s, c in cam_entries.items()
            if c["extrinsics"]["toCameraSocket"] == -1 or not c["extrinsics"]["rotationMatrix"]
        ]
        if len(ref_sockets) != 1:
            raise ValueError(f"{path}: expected exactly one reference camera," f" found sockets {ref_sockets}")
        ...
        if not imu_ext.get("rotationMatrix"):
            raise ValueError(f"{path}: imuExtrinsics missing/empty - cannot " "build extrinsic.imuFrame without it")
        if imu_ext["toCameraSocket"] != self.reference_socket:
            raise ValueError(
                f"{path}: imuExtrinsics.toCameraSocket "
        ...
        for sock, cam in cam_entries.items():
            ...
            if cam["cameraType"] == 1:
                model = (
                    "kb4",
                    dict(fx=K[0][0], fy=K[1][1], cx=K[0][2], cy=K[1][2], k1=dc[0], k2=dc[1], k3=dc[2], k4=dc[3]),
                )
            elif cam["cameraType"] == 0:
                model = ("ds", dict(fx=dc[5], fy=dc[6], cx=dc[7], cy=dc[8], xi=dc[9], alpha=dc[10]))
            else:
                raise ValueError(f"{path}: socket {sock} has unknown " f"cameraType {cam['cameraType']}")
            self.cams[sock] = {
                "model": model[0],
                "params": model[1],
                "width": cam["width"],
    ...
    def for_cam_name(self, cam_name):
        ...
        if sock not in self.cams:
            raise ValueError(
                f"{self.path}: no calibration for {cam_name} " f"(socket {sock}); sockets: {list(self.cams)}"
            )
...
class McapConverter:
    def __init__(
        ...
        exposure_usec=DEFAULT_EXPOSURE_USEC,
        gain=DEFAULT_GAIN,
    ):
        ...
        self.fps = float(fps if fps is not None else os.environ.get("PLAYGROUND_FPS", 30.0))
        self.start_time_ns = int(start_time_ns)
        self.exposure_usec = int(exposure_usec)
        self.gain = int(gain)
        calibration_file = calibration_file or os.environ.get("PLAYGROUND_CALIB")
        self.calibration = RigCalibration(calibration_file) if calibration_file else None
    ...
    def set_filepath(self):
        ...
        self.output_fullpath = os.path.join(self.output_folder, self.output_filename)
    ...
    def build_image_message(self, img_array, index, cam_name, stamp_ns):
        ...
        if w % 8:
            print(
                f"[mcap_convertor] WARNING: width {w} not divisible by 8; "
        ...
        if self.calibration is None:
            if not self._warned_no_calib:
                ...
                print(
                    "[mcap_convertor] WARNING: no calibration file given "
        ...
        for field, T in (("bodyFrame", cam["body_T_cam"]), ("imuFrame", cam["imu_T_cam"])):
            se3 = getattr(msg.extrinsic, field)
            se3.position.x, se3.position.y, se3.position.z = (float(v) for v in T[:3, 3])
...
def reconvert_mcap(conv, in_path, out_path, cams=None, max_frames=None):
    ...
    topics = sorted(ch.topic for ch in summary.channels.values() if ch.topic.startswith("S1/cam"))
    if cams:
        topics = [t for t in topics if "Cam" + t[-1].upper() in cams]
    if not topics:
    ...
    with open(out_path, "wb") as stream:
        ...
        for seq, cam_name, gray in merged:
            conv.write_cam_frame_to_mcap(writer, channels, cam_name, gray, seq, seq * interval)
            n += 1
            if n % 200 == 0:
...
def convert_dirs(conv, root, out_path, cams=None, max_frames=None):
    ...
    cams = cams or [c for c in CAMERA_NAMES if os.path.isdir(os.path.join(root, c))]
    if not cams:
    ...
    with open(out_path, "wb") as stream:
        ...
        for i in range(num_frames):
            for cam_name in cams:
                frame = conv.load_frame_from_png(i, os.path.join(root, cam_name))
                conv.write_cam_frame_to_mcap(writer, channels, cam_name, frame, i, i * interval)
        writer.finish()
...
def main():
    ...
    ap.add_argument("--calib", required=True, help="device JSON for the unit that made the frames")
    ...
    ap.add_argument("--fps", type=float, default=None, help="frame rate (default: PLAYGROUND_FPS env, else 30)")
    ap.add_argument("--start-time", type=float, default=1.0, help="stamp of frame 0 in seconds (imu_list must lead it)")
    ap.add_argument("--cams", nargs="*", help="subset of CamA..CamD (default: all found)")
    ap.add_argument("--max-frames", type=int)
    args = ap.parse_args()

    conv = McapConverter(calibration_file=args.calib, fps=args.fps, start_time_ns=int(args.start_time * 1e9))
    if args.from_mcap:
        reconvert_mcap(conv, args.from_mcap, args.output, cams=args.cams, max_frames=args.max_frames)
    else:
        convert_dirs(conv, args.from_dirs, args.output, cams=args.cams, max_frames=args.max_frames)
