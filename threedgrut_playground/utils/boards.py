...
def make_texture(gap_ratio, square, device, rows, cols, tag_order):
    if len(tag_order) < rows * cols:
        raise ValueError(f"tag_order holds {len(tag_order)} ids, " f"a {rows}x{cols} board needs {rows * cols}")
    ...
    tex = torch.tensor([1.0, 1.0, 1.0, 1.0], device=device, dtype=torch.float32).repeat(height, width, 1)
    tex = create_grid(square, gap, 0.0, rows, cols, tex)
    tex = _tag_pass(int(square * 0.5), gap, rows, cols, tex, int(square / 4), tag_order, device)
...
def build_materials(device, PBRMaterial, logo_path=None, square=800):
    ...
    for mid, (name, rows, cols, ids) in enumerate(BOARD_SPECS):
        ...
        materials[name] = mat(mid, make_texture(0.3, square, device, rows, cols, order))
    ...
    if logo_path and os.path.exists(logo_path):
        ...
        materials["vilota_logo"] = mat(len(BOARD_SPECS), torch.from_numpy(img).to(device=device, dtype=torch.float32))
    else:
...
def board_half_extents(name, tag_cm):
    for spec_name, rows, cols, _ids in BOARD_SPECS:
        if spec_name == name:
            return (
                (cols + 0.3 * (cols - 1) + 0.6) * tag_cm * 0.01 / 2.0,
...
def layout_positions(names, tag_cm, near, span, up_i=1, up_s=1.0, fw_i=2, fw_s=-1.0, square_cm=LAYOUT_SQUARE_CM):
...
def board_aabbs(placed, square_cm, up_i=1, fw_i=2):
    ...
    for name, pos in placed:
        ...
        out.append((name, (pos[side_i] - w / 2.0, pos[up_i] - h / 2.0), (pos[side_i] + w / 2.0, pos[up_i] + h / 2.0)))
    return out


def check_layout_no_overlap(names, tag_cm=5.173, near=None, span=0.6, square_cm=LAYOUT_SQUARE_CM):
        ...
        for j in range(i + 1, len(aabbs)):
            ...
            if all(amin[k] < bmax[k] and bmin[k] < amax[k] for k in range(2)):
                raise AssertionError(
                    f"{na} and {nb} overlap face-on at {square_cm} cm " f"squares: {amin}..{amax} vs {bmin}..{bmax}"
                )
    return aabbs
...
def spawn_scene(engine, primitive_type, device):
    ...
    if raw:
        ...
        for i, (name, ang) in enumerate(zip(names, angles)):
            ...
            placed.append((name, pos))
        print(f"[playground] BOARD_ANGLES set: angular layout, overlaps at " f"{LAYOUT_SQUARE_CM:.0f} cm squares")
    else:
        placed = layout_positions(names, tag_cm, near, span, up_i=up_i, up_s=up_s, fw_i=fw_i, fw_s=fw_s)

    step = vertical_step(names)
    print(
        f"[playground] {n} board(s), tag {tag_cm:.3f} cm, "
    ...
    for name, pos in placed:
        ...
        engine.primitives.add_primitive(
            geometry_type="Quad", primitive_type=primitive_type, device=device, material_name=name
        )
...
def spawn_from_file(engine, primitive_type, device, path):
    ...
    for entry in boards:
        ...
        if size is not None:
            obj.transform.sx, obj.transform.sy = size
        print(
            f"[playground]   {name:<12} pos ({pos[0]:+.2f}, {pos[1]:+.2f}, "
