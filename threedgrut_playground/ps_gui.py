...
class Playground:
    ...
    def init_polyscope(self, buffer_mode):
        ...
        try:
            win_w, win_h = (int(v) for v in os.environ.get("PLAYGROUND_WIN", "").lower().split("x"))
        except ValueError:
    ...
    def render_from_current_ps_view(self, window_w=None, window_h=None, distortion=None):
        ...
        camera = polyscope_to_kaolin_camera(
            view_params, window_w, window_h, distortion_coefficients=distortion, device=self.engine.device
        )
    ...
    def update_render_view_viz(self, force=False):
        ...
        try:
            sple_orad, sple_odns = self.render_from_current_ps_view(
                distortion=distortions if self.distortions else None
            )
        ...
        if psim.TreeNode("Record Trajectory Video"):
            ...
            if psim.Button("Add Camera"):
                if self.selected_camera_idx is None:
                    ...
                    self.add_cam_to_vid_recorder(self.selected_camera_idx)
            ...
            if psim.Button("Render Video"):
                try:
                    ...
                    self.video_recorder.render_video(
                        cam_name=cam_name if self.novel_view_renderer.is_loaded() else None
                    )
    ...
    def _draw_novel_view_from_calib_controls(self):
        ...
        if psim.TreeNode("Novel View from Vilota Calibration file"):
            ...
            if not hasattr(self, "_calib_files"):
                ...
                self._calib_idx = self._calib_files.index(cur) if cur in self._calib_files else 0
            if self._calib_files:
                changed, self._calib_idx = psim.Combo("Calibration file", self._calib_idx, self._calib_labels)
                if changed:
                    self.novel_view_renderer.calibration_filename = self._calib_files[self._calib_idx]
                psim.SameLine()
            ...
            _, self.novel_view_renderer.calibration_filename = psim.InputText(
                "Calib path", self.novel_view_renderer.calibration_filename
            )
            ...
            if psim.Button("Load Calibration"):
                try:
                    ...
                    self._novel_view_calib_status = (
                        f"Loaded {self.novel_view_renderer.get_camera_count()} cameras from {calibration_path}"
                    )
            ...
            if self.novel_view_renderer.is_loaded():
                ...
                if psim.Button("Reload Calibration"):
                    try:
                        ...
                        self._novel_view_calib_status = (
                            f"Reloaded {self.novel_view_renderer.get_camera_count()} cameras from {calibration_path}"
                        )
                        self.distortions = self.novel_view_renderer.get_cam_distortions()
            ...
            if self.calibration_loaded:
                if psim.TreeNode("Device loaded"):
                    ...
                    _, self.flyto_relative = psim.Checkbox(
                        "Fly to relative to current view", getattr(self, "flyto_relative", False)
                    )
                ...
                if psim.TreeNode("Save/Load Video trajectory"):
                    ...
                    _, self.novel_view_renderer.trajectory_filename = psim.InputText(
                        "Trajectory file", self.novel_view_renderer.trajectory_filename
                    )
                    ...
                    _, self.traj_type_idx = psim.Combo("Trajectory type", self.traj_type_idx, traj_types)
                    psim.PopItemWidth()
                    psim.SameLine()
                    if psim.Button("Build Trajectory"):
                        ...
                        try:
                            self.poses = build_orbit_trajectory(
                                self, flip=getattr(self, "orbit_flip", False), vio=(self.traj_type_idx == 1)
                            )
                            ...
                            if self.poses:
                                first = getattr(self, "orbit_first_pose", {}).get(self.selected_camera_idx, 0)
                    ...
                    psim.SameLine()
                    _, self.orbit_flip = psim.Checkbox("Flip side", getattr(self, "orbit_flip", False))
                    ...
                    if self.trajectory_loaded:
                        if psim.Button("Add current view to trajectory"):
                            ...
                            self.poses = self.novel_view_renderer.add_pose_to_trajectory(
                                current_cam_view_mat, curr_cam_index
                            )
                        if psim.TreeNode(f"Trajectory: {len(self.poses)} poses"):
                            _, self.export_cam_index = psim.SliderInt(
                                "Choose Cam",
                                self.export_cam_index,
                                v_min=0,
                                v_max=self.novel_view_renderer.get_camera_count() - 1,
                            )
    ...
    def _scan_calibration_files(self):
        ...
        for fn in found:
            try:
                ...
                label = f"{fn}  [{dev}, {cams} cams]" if cams else f"{fn}  [no cameraData]"
            except Exception:
    ...
    def _draw_transform_widget(self, obj):
        ...
        if psim.TreeNode("Transform"):
            ...
            _, self.square_size = psim.InputFloat("Set Square Size:", self.square_size, format="%.2f")
            psim.SameLine()
            ...
            _, tvals = psim.InputFloat3(
                "Translate (exact)", [object_transform.tx, object_transform.ty, object_transform.tz], format="%.4f"
            )
            ...
            _, rvals = psim.InputFloat3(
                "Rotate (exact)", [object_transform.rx, object_transform.ry, object_transform.rz], format="%.3f"
            )
    ...
    def calculate_sx_and_sy(self, square: float, rows: int = 4, cols: int = 7):
        ...
        """
        return (
            (cols + 0.3 * (cols - 1) + 0.6) * square * 0.01 / 2.0,
