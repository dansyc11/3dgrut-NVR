...
class Primitives:
    ...
    def register_default_materials(self, device) -> Dict[str, PBRMaterial]:
        ...
        return build_materials(
            device, PBRMaterial, os.path.join(os.path.dirname(__file__), "..", "assets", "vilota_logo.png")
        )
    ...
    def add_primitive(
        self, geometry_type: str, primitive_type: OptixPrimitiveTypes, device, material_name: str = None
    ) -> None:
        ...
        if material_name is not None:
            if material_name not in self.registered_materials:
                raise KeyError(f"no material named {material_name}. " f"Have: {sorted(self.registered_materials)}")
            mesh.material_assignments = torch.full_like(
                mesh.material_assignments, self.registered_materials[material_name].material_id
            )
