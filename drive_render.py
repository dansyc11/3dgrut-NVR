...
# Run from the repo root (this file's folder): every path below is repo-relative.
...
for i in range(n_cams):
    ...
    log(
        f"  cam{i} {pg.novel_view_renderer.get_cam_name_at_index(i)} "
...
log(f"quad '{key}' reset: sx={tr.sx} sy={tr.sy} (aspect {tr.sx/tr.sy:.4f}, " f"texture aspect 752/440={752/440:.4f})")
...
nz = np.sort(ext)[::-1][:2]
log(
    f"quad world extent {ext} -> {nz[0]:.4f} x {nz[1]:.4f} m, "
...
for idx, nm in enumerate(names):
    ...
    log(
        f"  {nm} idx{idx}: fx={fx:.3f}(file {rK[0][0]:.3f}) "
